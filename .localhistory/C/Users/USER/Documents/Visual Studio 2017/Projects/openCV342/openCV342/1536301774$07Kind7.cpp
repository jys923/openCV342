...
int FindPolygon()
{
	...
	img_input = imread("./../../[00Images]/etc/test.bmp", IMREAD_COLOR);
	if (img_input.empty())
	{
	...
	//이진화 이미지로 변환
	Mat binary_image;
	//threshold(img_gray, img_gray, 250, 255, THRESH_BINARY_INV | THRESH_OTSU);
	adaptiveThreshold(img_gray,img_gray,255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 100, 10);
