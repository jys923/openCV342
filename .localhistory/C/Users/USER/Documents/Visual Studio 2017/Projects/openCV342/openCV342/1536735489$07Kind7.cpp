...
int testUpperLib()
{
	...
	//이진화 이미지로 변환
	Mat binary_image, binary_image2, img_result, img_result2, canny_image2, canny_image3;
	binary_image = img_gray.clone();
	...
	for (int i = 10; i > 0; i--)
	{
		...
		HoughLines(canny_image3, linesL, 1, CV_PI / 180, threshold, 0, 0, CV_PI / 2, CV_PI);
		if (linesL.size() > 0)
		{
			for (size_t i = 0; i < linesL.size(); i++)
			{
				...
				cv::line(img_result3, pt1, pt2, Scalar(0, 0, 255));
			}
			break;
		}
	}

	imshow("right", img_result3);
	waitKey(0);
	// 오른쪽 차선 검출
	/*for (int i = 10; i > 0; i--)
	{
