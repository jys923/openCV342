...
int testUpperLib()
{
	...
	adaptiveThreshold(binary_image, canny_image, 230, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 11, 10);
	...
	waitKey(0);
	hconcat(img_input, img_input, img_result); //두 이미지의 높이(row)가 같아야함.
	imshow("result", img_result);
	waitKey(0);
