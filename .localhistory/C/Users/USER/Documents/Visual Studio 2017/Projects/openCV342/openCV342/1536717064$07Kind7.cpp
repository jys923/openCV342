...
int testUpperLib()
{
	...
	for (size_t i = 0; i < 255; i++)
	{
		...
		adaptiveThreshold(binary_image, canny_image, 230, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 15, -2);
		imshow("Threshold", canny_image);
		waitKey(0);

		int horizontalsize = canny_image.cols / 30;

		Mat structure = getStructuringElement(MORPH_RECT, Size(horizontalsize, 3));
