...
int testUpperLib()
{
	...
	for (size_t i = 0; i < 255; i++)
	{
		...
		//waitKey(0);
		Mat canny_image;
		blur(binary_image, canny_image, Size(7, 7));
		//adaptiveThreshold(canny_image, canny_image, 230, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 55, 10);
		Canny(canny_image, canny_image, 30, 30 * 3, 3);
		imshow("Canny", canny_image);
		waitKey(0);
		//직선 성분을 검출합니다.  
		vector<Vec4i> linesL, linesR, lines;
		//HoughLinesP(canny_image, lines, 1, 1*CV_PI / 180 ,10, 0, 0);
		// 왼쪽 
		HoughLines(canny_image, linesL, 1, 1 * CV_PI / 180, 10, canny_image.cols / 3, 0, 0, CV_PI / 2);
		// 오른쪽 
		HoughLines(canny_image, linesR, 1, 1 * CV_PI / 180, 10, canny_image.cols / 3, 0, 0, CV_PI / 2, CV_PI);
