...
int FindPolygon()
{
	...
	for (size_t i = 0; i < 255; i++)
	{
		...
		waitKey(0);
		Mat canny_image;
		blur(binary_image2, canny_image, Size(3, 3));
		Canny(canny_image, canny_image, 125, 125 * 3, 3);
		imshow("Canny", canny_image);
		waitKey(0);
		vector<Vec3f> circles;
		HoughCircles(canny_image, circles, HOUGH_GRADIENT, 1, canny_image.rows / 8, 200, 100, 0, 0);
		//HoughCircles(canny_image, circles, HOUGH_GRADIENT, 2, canny_image.rows / 4, 50, 150);
